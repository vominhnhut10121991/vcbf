...
&& in_array($extension, $allowedExts)) 
{
  ...
  { 
	...
	for ($i = 0; $i < $total_sheets; $i++)
	{
		...
		{
			if (trim($structure[0]) != "Date" || trim($structure[1]) != "Industry" || trim($structure[2]) != "Industry_VN" || trim($structure[3])
			!= "Percentage")
			{
				echo "Sheet " . ($i + 1) . " is not 'Portfolio Breakdown'. Please, <a href='index.php'> try again </a> </br>";
				return;
			}
		}
		else
		...
		foreach ($arraydata as $row)
		{
			...
			$update = false;
			$r_sql = "select * from vcbf_portfolio_industry_breakdown_tbf";
			foreach (select($pdo, $r_sql) as $db_row) 
			{ 
				if (($db_row[0] == $date && $db_row[1] == $industry ) || ($db_row[0] == $date && $db_row[2] == $industry_vn ))
				{
					$update = true;
					break;
				}
			...
			if ($update ==  true)
			{
				$sql = "UPDATE vcbf_portfolio_industry_breakdown_tbf SET percentage = " . $percentage . " WHERE date = '" . $date . "' and (industry = 
					'" . $industry . "' or industry_vn = '" . $industry_vn . "') ";
			...
			{
				$sql = "INSERT INTO vcbf_portfolio_industry_breakdown_tbf VALUES ('" . $date . "', '" . $industry . "' , '" . $industry_vn . "' ,
					" . $percentage . ")";
