...
&& in_array($extension, $allowedExts)) 
{
  ...
  { 
	...
	for ($i = 0; $i < $total_sheets; $i++)
	{
		...
		{
			if (trim($structure[0]) != "Weeks" || trim($structure[1]) != "NAV per unit" )
			{
				echo "Sheet " . ($i + 1) . " is not 'NAV'. Please, <a href='index.php'> try again</a> </br>";
				return;
			}
		}
		else
		...
		{
			$week = date('Y-m-d',unixstamp($row[0]));
			if(!preg_match("/^(?:20|19)[0-9]{2}([-.\\/])(?:0?[1-9]|1[012])\\1(?:0?[1-9]|[12][0-9]|3[01])$/", $week)) 
			{ 
			...
			if(!is_numeric ($nav))
			{
				echo "NAV of row " . $line . " in sheet " . ($i + 1) . " is not number. </br>";
				continue;
			}
			...
			$update = false;
			$r_sql = "select * from vcbf_nav_bcf";
			...
			if ($update ==  true)
			{
				$sql = "UPDATE vcbf_nav_bcf SET nav = " . $nav . " WHERE week = '" . $week . "'";
			...
			{
				$sql = "INSERT INTO vcbf_nav_bcf(week, nav) VALUES ('" . $week . "'," . $nav . ")";
