...
&& in_array($extension, $allowedExts)) 
{
  ...
  { 
	...
	for ($i = 0; $i < $total_sheets; $i++)
	{
		...
		{
			if (trim($structure[0]) != "Date" || trim($structure[1]) != "Asset" || trim($structure[2]) != "Asset_VN" || trim($structure[3])
			!= "Percentage")
			{
				echo "Sheet " . ($i + 1) . " is not 'Portfolio Details'. Please, <a href='index.php'> try again </a> </br>";
				return;
			}
		}
		else
		...
		foreach ($arraydata as $row)
		{
			...
			$update = false;
			$r_sql = "select * from vcbf_portfolio_detail_tbf";
			foreach (select($pdo, $r_sql) as $db_row) 
			{ 
				if (($db_row[0] == $date && $db_row[1] == $asset ) || ($db_row[0] == $date && $db_row[2] == $asset_vn ))
				{
					$update = true;
					break;
				}
			...
			if ($update ==  true)
			{
				$sql = "UPDATE vcbf_portfolio_detail_tbf SET percentage = " . $percentage . " WHERE date = '" . $date . "' and (asset = '" . $asset . "' 
					or asset_vn = '" . $asset_vn . "')";
			...
			{
				$sql = "INSERT INTO vcbf_portfolio_detail_tbf VALUES ('" . $date . "', '" . $asset . "', '" . $asset_vn . "', " . $percentage . ")";
