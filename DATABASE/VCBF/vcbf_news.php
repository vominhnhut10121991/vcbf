...
&& in_array($extension, $allowedExts)) 
{
  ...
  { 
	...
	for ($i = 0; $i < $total_sheets; $i++)
	{
		...
		{
			if (trim($structure[0]) != "Header" || trim($structure[1]) != "Content" || trim($structure[2]) != "Header_VN" || trim($structure[3])
			!= "Content_VN" || trim($structure[4]) != "Date")
			{
				echo "Sheet " . ($i + 1) . " is not 'news'. Please, <a href='index.php'> try again </a> </br>";
				return;
			}
		}
		else
		...
		foreach ($arraydata as $row)
		{
			...
			$update = false;
			$r_sql = "select * from vcbf_news";
			foreach (select($pdo, $r_sql) as $db_row) 
			{ 
				if ($db_row[1] == $header || $db_row[3] == $header_vn)
				{
					$update = true;
					break;
				}
			...
			if ($update ==  true)
			{
				$sql = "UPDATE vcbf_news SET header = '" . $header . "', content = '" . $content . "', header_vn = '" . $header_vn . "', 
					content_vn = '" . $content_vn . "', date = '" . $date . "' WHERE header = '" . $header . "' or header_vn = '" . $header_vn . "'" ;
			...
			{
				$sql = "INSERT INTO vcbf_news(header, content, header_vn, content_vn, date) VALUES 
					('" . $header . "','" . $content . "' , '" . $header_vn . "','" . $content_vn . "' ,'" . $date . "' )";
