  ...
  { 
	...
	for ($i = 0; $i < $total_sheets; $i++)
	{
		...
		if (sizeof($structure) == 5)
		{
			...
			{
				echo "Sheet " . ($i + 1) . " is not 'Reminding Message'. Please, <a href='index.php'> try again </a> </br>";
				return;
			}
		}
		else
		...
		foreach ($arraydata as $row)
		{
			...
			$update = false;
			$r_sql = "select * from vcbf_reminding_message";
			...
			if ($update ==  true)
			{
				$sql = "UPDATE vcbf_reminding_message SET title = '" . $title . "', content = '" . $content . "', title_vn = '" . $title_vn . "', content_vn = '" . $content_vn . "', date = '" . $date . "' WHERE title = '" . $title . "' or title_vn = '" . $title_vn . "'";
			...
			{
				$sql = "INSERT INTO vcbf_reminding_message(title, content, title_vn, content_vn, date) VALUES ('" . $title . "','" . $content . "' , '" . $title_vn . "','" . $content_vn . "', '" . $date . "' )";
